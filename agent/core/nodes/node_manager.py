...
class NodeManager:
    ...
    def planner_node(self, state):
        ...
        self.logger.info(f"[DEBUG] planner_node接收到的draft_outputs: {list(draft_outputs.keys()) if draft_outputs else 'Empty'}")
        self.logger.info(f"[DEBUG] planner_node接收到的draft_outputs数量: {len(draft_outputs)}")
