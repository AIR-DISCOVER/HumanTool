...
class RouterProcessor:
    ...
    def process(self, state: SimplerAgendaState) -> Dict[str, Any]:
        ...
        # 🔍 新增调试：记录接收到的状态信息
        self.logger.info(f"🔍 RouterProcessor - 接收状态对象ID: {id(state)}")
        self.logger.info(f"🔍 RouterProcessor - 接收状态_llm_response_content存在: {bool(state.get('_llm_response_content'))}")
        if state.get('_llm_response_content'):
            self.logger.info(f"🔍 RouterProcessor - 接收状态LLM内容长度: {len(state['_llm_response_content'])} 字符")
