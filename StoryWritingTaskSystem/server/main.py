...
async def get_user_profiles():
    ...
    try:
        ...
        # 如果数据库没有数据或连接失败，使用创意写作用户档案
        if not profiles:
            profiles = [
                {
                    'id': 'user_main',
                    'name': '通用写作者',
                    'description': '普通用户，喜欢创意写作和故事创作'
                },
                {
                    'id': 'user_student',
                    'name': '学生小李',
                    'description': '大学生，喜欢写日记和创意短文'
                },
                {
                    'id': 'user_office',
                    'name': '白领小王',
                    'description': '上班族，业余时间喜欢写作放松'
                },
                {
                    'id': 'user_parent',
                    'name': '家长老张',
                    'description': '家庭主妇/主夫，喜欢写家庭故事'
                },
                {
                    'id': 'user_retiree',
                    'name': '退休老师',
                    'description': '退休人员，有丰富人生阅历'
                }
            ]
        ...
        return [{'id': 'user_main', 'name': '通用写作者', 'description': '普通用户，对创意写作感兴趣'}]
...
async def chat_stream(request: ChatRequest):
    ...
    try:
        ...
        if user_id not in user_agents:
            user_agents[user_id] = {}
        if session_id not in user_agents[user_id]:
            agent = streaming_wrapper.create_agent(
                verbose=True, user_name=user_id, database_manager=database_manager
            )
            user_agents[user_id][session_id] = {"agent": agent, "created_at": time.time()}
        
        agent = user_agents[user_id][session_id]["agent"]
